...
int strtol(char *s, char **endp, int base);	/* 标准函数，在stdlib.h中 */
...
err:
				api_putstr0(" >teditor file [-w30 -h10 -t4]\n");
				api_end();
			}
		} else {	/* 找到文件名 */
			if (q != 0) {
	...
	/* 准备窗口 */
	win = api_openwin(winbuf, w * 8 + 16, h * 16 + 37, -1, "teditor");
	...
	*r = 0;
	...
	api_read(i, txtbuf + 1, j);
	api_fclose(i);
	txtbuf[j + 1] = 0;
	q = txtbuf + 1;
	for (p = txtbuf + 1; *p != 0; p++) {	/* 为了让处理变得简单，删掉0x0d的代码（回车符号）*/
		if (*p != 0x0d) {
			*q = *p;
			q++;
		}
